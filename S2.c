...
void replace_path(const char *input, char *output, size_t outsize) {
    ...
    const char *replacement1 = "~S2/";
    const char *replacement2 = "S2/";
    const char *replacement3 = "~S2";
    const char *replacement4 = "S2";
...
void removef(int socket){
  ...
  if (mod_path[0] == '~') {
    ...
    fflush(stdout);
  if(remove(temp)==0) {
      printf("File %s successfully deleted from Server\n",filename);
      fflush(stdout);
  }
...
void download_tarf(int socket){
    ...
    const char *home = getenv("HOME");
    snprintf(root, sizeof(root), "%s/S2", home);
...
int main(){ // Server 2 (pdf Server)
    int listening_socket, connection_socket;
    struct sockaddr_in server_address; //Socket address object to which IP address and port number are added
    ...
    while (1){
        connection_socket= accept(listening_socket,NULL,NULL); //accept()
